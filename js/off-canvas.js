...
var SidebarMenuEffects = (function () {
    ...
    function init() {
        ...
        var container = document.getElementById('oc-container'),
			...
			menuClickFn = function (evt) {
			    if (hasParentClass(evt.target, 'oc-menu-tier-2')) {
			        setTimeout(function () {
			            resetMenu();
			        }, 500);
			        document.removeEventListener(eventtype, bodyClickFn);
        ...
        buttons.forEach(function (el, i) {
            ...
            el.addEventListener(eventtype, function (ev) {
                ...
                $('.oc-nav').css("display", "block"); // ensures OC nav is outside of tabindex scope
                setTimeout(function () {
                    classie.add(container, 'oc-nav-open');
                    $('#ic-env').hide();
                }, 25);
                document.addEventListener(eventtype, bodyClickFn);
                document.addEventListener(eventtype, menuClickFn);
